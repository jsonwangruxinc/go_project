...
type GreeterClient interface {
	...
	// 服务端流，下载文件场景
	SayHelloToWayStream(ctx context.Context, in *HelloRequest, opts ...grpc.CallOption) (Greeter_SayHelloToWayStreamClient, error)
}
...
func (c *greeterClient) SayHelloClientStream(ctx context.Context, opts ...grpc.CallOption) (Greeter_SayHelloClientStreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &Greeter_ServiceDesc.Streams[0], "/helloword.Greeter/SayHelloClientStream", opts...)
	if err != nil {
		return nil, err
	}
...
func (c *greeterClient) SayHelloToWayStream(ctx context.Context, in *HelloRequest, opts ...grpc.CallOption) (Greeter_SayHelloToWayStreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &Greeter_ServiceDesc.Streams[1], "/helloword.Greeter/SayHelloToWayStream", opts...)
	if err != nil {
		return nil, err
	}
	x := &greeterSayHelloToWayStreamClient{stream}
...
type Greeter_SayHelloToWayStreamClient interface {
	Recv() (*HelloReply, error)
	grpc.ClientStream
}

type greeterSayHelloToWayStreamClient struct {
	grpc.ClientStream
}

func (x *greeterSayHelloToWayStreamClient) Recv() (*HelloReply, error) {
	m := new(HelloReply)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
type GreeterServer interface {
	...
	// 服务端流，下载文件场景
	SayHelloToWayStream(*HelloRequest, Greeter_SayHelloToWayStreamServer) error
...
func (UnimplementedGreeterServer) SayHelloToWayStream(*HelloRequest, Greeter_SayHelloToWayStreamServer) error {
	return status.Errorf(codes.Unimplemented, "method SayHelloToWayStream not implemented")
}
...
func _Greeter_SayHelloToWayStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(HelloRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(GreeterServer).SayHelloToWayStream(m, &greeterSayHelloToWayStreamServer{stream})
}

type Greeter_SayHelloToWayStreamServer interface {
	Send(*HelloReply) error
	grpc.ServerStream
}

type greeterSayHelloToWayStreamServer struct {
	grpc.ServerStream
}

func (x *greeterSayHelloToWayStreamServer) Send(m *HelloReply) error {
	return x.ServerStream.SendMsg(m)
}
...
var Greeter_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "SayHelloToWayStream",
			Handler:       _Greeter_SayHelloToWayStream_Handler,
			ServerStreams: true,
		},
	},
